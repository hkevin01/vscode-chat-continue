...
def cleanup_existing_processes():
    ...
    try:
        ...
        if killed_count > 0:
            print(f"   📊 Cleaned up {killed_count} existing automation "
                  f"process(es)")
        else:
...
def create_parser():
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Run in dry-run mode (no actual clicking)'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=8.0,
        help='Detection interval in seconds (default: 8.0)'
    )
    ...
    try:
        while True:
            ...
            if clicks > total_clicks:
                ...
                print(f"   ✅ Clicked {new_clicks} Continue button(s)! "
                      f"Total: {clicks}")
                total_clicks = clicks
            else:
