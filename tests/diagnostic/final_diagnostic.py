...
def main():
    ...
    try:
        ...
        result = subprocess.run([
            "bash", "-c",
            f"xwd -id {cv_window['id']} | convert xwd:- {image_path}"
        ], capture_output=True)
        ...
        if buttons:
            ...
            for i, button in enumerate(buttons):
                ...
                passes_strict = (button.x > chat_left and
                               button.y > max(chat_bottom, avoid_top) and
            ...
            draw.rectangle([chat_left, max(chat_bottom, avoid_top),
        ...
        if buttons:
            strict_count = sum(1 for b in buttons 
                             if b.x > int(image.width * 0.55) and 
                                b.y > max(int(image.height * 0.75), int(image.height * 0.1)))
            print(f"   Pass strict filter: {'✅' if strict_count > 0 else '❌'} {strict_count} buttons")
