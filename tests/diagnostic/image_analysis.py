...
def analyze_image(image_path: str):
    ...
    preprocessing_methods = [
        ...
        (
            ...
            cv2.adaptiveThreshold(
                center_region, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2
            ),
        ),
    ]
                ...
                if text:
                    ...
                    if lines:
                        ...
                        for line in lines:
                            if any(
                                word in line.lower()
                                for word in ["continue", "cancel", "next", "proceed"]
                            ):
    ...
    for contour in contours:
        ...
        if 50 < w < 200 and 20 < h < 60 and 1.5 < aspect_ratio < 6 and area > 500:
            button_candidates.append((x, y, w, h, area))

    print(f"🔲 Found {len(button_candidates)} button-like shapes:")
    for i, (x, y, w, h, area) in enumerate(
        sorted(button_candidates, key=lambda x: x[4], reverse=True)
    ):
...
def main():
    ...
    if not HAS_TESSERACT:
        ...
        try:
            ...
            result = subprocess.run(
                ["sudo", "apt", "install", "-y", "tesseract-ocr"], capture_output=True, text=True
            )
    ...
    debug_images = ["tmp/debug_attempt_1.png", "tmp/debug_attempt_2.png", "tmp/debug_attempt_3.png"]
