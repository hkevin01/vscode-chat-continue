...
def main():
    ...
    print(
        f"Window geometry: {window['width']}x{window['height']} "
    ...
    try:
        ...
        for i, btn in enumerate(ocr_buttons):
            print(
                f"   {i+1}. ({btn.x},{btn.y}) {btn.width}x{btn.height} "
    ...
    for btn in all_buttons:
        in_area = (
            btn.x > chat_panel_left
            and btn.y > max(chat_panel_bottom, avoid_top)
            and btn.y < image.height - 20
        )
    ...
    if all_buttons:
        ...
        draw.rectangle(
            [chat_panel_left, max(chat_panel_bottom, avoid_top), image.width, image.height - 20],
            outline="yellow",
            width=3,
        )
        ...
        for i, button in enumerate(all_buttons):
            in_area = (
                button.x > chat_panel_left
                and button.y > max(chat_panel_bottom, avoid_top)
            ...
            # Add label
            draw.text((button.x, button.y - 15), f"{i+1}", fill=color)
