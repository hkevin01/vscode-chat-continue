                    ...
                    for config_name, config in ocr_configs:
                        try:
                            ...
                            if "continue" in text.lower():
                                continue_matches = [
                                    w for w in text.split() if "continue" in w.lower()
                                ]
...
try:
    ...
    if screenshot:
        ...
        if windows:
            for i, window in enumerate(windows):
                try:
                    window_img = screen.capture_region(
                        window.x, window.y, window.width, window.height
                    )
                    if window_img and window_img.size[0] > 100:
                        window_buttons = finder.find_continue_buttons(
                            window_img, window.x, window.y
                        )
                        print(f"   Window {i+1} buttons: {len(window_buttons)}")

                        for j, btn in enumerate(window_buttons):
                            print(
                                f"     W{i+1}B{j+1}: ({btn.x},{btn.y}) method={btn.method} conf={btn.confidence:.2f}"
                            )
                except:
                    pass
