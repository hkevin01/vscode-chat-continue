    ...
    for window in windows:
        ...
        try:
            ...
            if buttons:
                ...
                good_buttons = []
                for btn in buttons:
                    print(f"   • Text: '{btn.text}' | Confidence: {btn.confidence:.2f} | Method: {btn.method}")
                    
                    # Consider it a good button if:
                    # 1. High confidence (>0.7), OR
                    # 2. Contains "continue" text, OR  
                    # 3. Blue button detection method
                    if (btn.confidence > 0.7 or 
                        (btn.text and 'continue' in btn.text.lower()) or
                        'blue' in btn.method.lower()):
