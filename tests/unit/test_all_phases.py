...
class TestPhase2CoreFeatures(unittest.TestCase):
    ...
    def test_multi_window_support(self):
        ...
        mock_windows = [
            VSCodeWindow(
                window_id=1, title="VS Code - test1", x=0, y=0, width=800, height=600, pid=123
            ),
            VSCodeWindow(
                window_id=2, title="VS Code - test2", x=100, y=100, width=800, height=600, pid=124
            ),
        ]

        with patch.object(engine.window_detector, "get_vscode_windows", return_value=mock_windows):
    ...
    def test_basic_error_handling(self):
        ...
        try:
            ...
            with patch.object(
                engine.window_detector, "get_vscode_windows", side_effect=Exception("Test error")
            ):
...
class TestPhase3Enhancement(unittest.TestCase):
    ...
    def test_configuration_system(self):
        ...
        # Create test config
        config_data = {
            "automation": {"interval_seconds": 2.0, "max_retries": 3, "dry_run": True},
            "detection": {
                "confidence_threshold": 0.8,
                "button_variants": ["Continue", "Continue >"],
            },
            "safety": {"emergency_stop_key": "escape", "pause_on_user_activity": True},
        }
        with open(self.config_path, "w") as f:
            json.dump(config_data, f)
...
class TestPhase4Polish(unittest.TestCase):
    ...
    def test_installation_scripts(self):
        ...
        for script_name in expected_scripts:
            ...
            if os.name != "nt":  # Not Windows
                self.assertTrue(
                    os.access(script_path, os.X_OK), f"Script {script_name} should be executable"
                )
    ...
    def test_documentation_and_tutorials(self):
        ...
        for doc_name in expected_docs:
            ...
            self.assertGreater(len(content), 100, f"Doc {doc_name} should have substantial content")
    ...
    def test_testing_and_bug_fixes(self):
        ...
        self.assertTrue(comprehensive_test.exists(), "Comprehensive test suite should exist")
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def setUp(self):
        ...
        config_data = {
            "automation": {"interval_seconds": 1.0, "max_retries": 2, "dry_run": True},
            "detection": {
                "confidence_threshold": 0.8,
                "button_variants": ["Continue", "Continue >"],
            },
            "fallback_strategy": {"enabled": True, "text_commands": ["continue"], "max_retries": 2},
            "safety": {"emergency_stop_key": "escape", "pause_on_user_activity": True},
            "logging": {"level": "INFO", "file_path": str(Path(self.temp_dir) / "integration.log")},
        }
        with open(self.config_path, "w") as f:
            json.dump(config_data, f)
...
def run_all_tests():
    ...
    test_suites = [
        ...
        (
            "Phase 2: Core Features",
            unittest.TestLoader().loadTestsFromTestCase(TestPhase2CoreFeatures),
        ),
        (
            "Phase 3: Enhancement",
            unittest.TestLoader().loadTestsFromTestCase(TestPhase3Enhancement),
        ),
        ("Phase 4: Polish", unittest.TestLoader().loadTestsFromTestCase(TestPhase4Polish)),
        (
            "Integration Tests",
            unittest.TestLoader().loadTestsFromTestCase(TestIntegrationScenarios),
        ),
    ]
