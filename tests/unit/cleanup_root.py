...
def main():
    ...
    for item in root_dir.iterdir():
        if item.is_file() and not item.name.startswith(".") and item.name not in keep_in_root:
            ...
            if filename.startswith("test_automation") or filename == "quick_automation_test.py":
                ...
                filename.startswith("cleanup_")
                or filename.startswith("organize_")
                or filename == "final_cleanup.sh"
            ):
    ...
    remaining_files = [f for f in root_dir.iterdir() if f.is_file() and not f.name.startswith(".")]
