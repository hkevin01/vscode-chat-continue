...
async def setup_and_test():
    ...
    image = engine.screen_capture.capture_window(
        test_window.window_id, test_window.x, test_window.y, test_window.width, test_window.height
    )
    ...
    try:
        ...
        if sys.platform.startswith("linux"):
            subprocess.run(["wmctrl", "-a", "Mozilla Firefox"], capture_output=True, text=True)
        time.sleep(1)
