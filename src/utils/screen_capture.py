...
class ScreenCapture:
    ...
    def __init__(self):
        ...
        try:
            if self.platform == "Linux":
                ...
                # Disable X11 bell
                subprocess.run(["xset", "-b"], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
        except Exception:
        ...
        if self.platform == "Linux":
            ...
            try:
                result = subprocess.run(
                    ["which", "scrot"], capture_output=True, stderr=subprocess.DEVNULL
                )
            ...
            try:
                result = subprocess.run(
                    ["which", "import"], capture_output=True, stderr=subprocess.DEVNULL
                )
                if result.returncode == 0:
    ...
    def capture_screen(self) -> Optional[Image.Image]:
        ...
        try:
            ...
            if self.platform == "Linux":
                ...
                if wayland_display:
                    self.logger.debug(
                        "Detected Wayland, using mock screenshot to prevent freezing..."
                    )
    ...
    def _capture_screen_wayland(self) -> Optional[Image.Image]:
        ...
        self.logger.warning("Wayland screenshot capture disabled due to system conflicts")
        self.logger.warning("Automation will work without screenshots (using fallback detection)")
        ...
        try:
            ...
            self.logger.debug("Created mock screenshot for Wayland compatibility")
            return mock_image
        except Exception as e:
            ...
            try:
                with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
                    ...
                    if result.returncode == 0 and Path(tmp.name).exists():
                        img = Image.open(tmp.name)
                        Path(tmp.name).unlink()  # Clean up
                        self.logger.debug(f"✅ ImageMagick successful: {img.size}")
                        return img
                    else:
    ...
    def capture_region(self, x: int, y: int, width: int, height: int) -> Optional[Image.Image]:
        ...
        try:
            ...
            if self.platform == "Linux":
                ...
                if wayland_display:
                    self.logger.debug(
                        "Detected Wayland, using mock region capture " "to prevent freezing..."
                    )
                    return self._capture_screen_wayland()
            ...
            if self.platform == "Linux":
                ...
                if imagemagick_result and self._validate_screenshot(
                    imagemagick_result, width // 2, height // 2
                ):
                    return imagemagick_result

                scrot_result = self._capture_with_scrot(x, y, width, height)
                if scrot_result and self._validate_screenshot(
                    scrot_result, width // 2, height // 2
                ):
                    return scrot_result

                # Fallback for Linux: capture full screen and crop
                self.logger.debug(
                    "Linux-native region capture failed. " "Attempting to crop a full screenshot."
                )
            ...
            self.logger.error(f"An unexpected error occurred during region " f"capture {bbox}: {e}")
            return None

    def _capture_with_scrot(self, x: int, y: int, width: int, height: int) -> Optional[Image.Image]:
        ...
        try:
            with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
                ...
                if result.returncode == 0:
                    ...
                    self.logger.debug(
                        f"scrot failed with code {result.returncode}, stderr: {result.stderr.decode()}"
                    )
                    Path(tmp.name).unlink(missing_ok=True)
                    return None
        ...
        try:
            with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
                ...
                if result.returncode == 0:
                    ...
                    self.logger.debug(
                        f"ImageMagick import failed with code {result.returncode}, stderr: {result.stderr.decode()}"
                    )
                    Path(tmp.name).unlink(missing_ok=True)
                    return None
    ...
    def _validate_screenshot(
        self, screenshot: Optional[Image.Image], min_width: int = 100, min_height: int = 100
    ) -> bool:
        ...
        if width < min_width or height < min_height:
            self.logger.warning(
                f"Screenshot too small: {width}x{height} (min: {min_width}x{min_height})"
            )
            return False
