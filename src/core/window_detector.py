...
class WindowDetector:
        ...
        try:
            for proc in psutil.process_iter(["pid", "name", "exe", "cmdline"]):
                try:
                    ...
                    if any(
                        vscode_name in name
                        for vscode_name in ["code", "vscode", "code-oss", "codium", "cursor"]
                    ) or any(
                        vscode_name in exe
                        for vscode_name in ["code", "vscode", "code-oss", "codium", "cursor"]
                    ):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    self.logger.warning(
                        "X11 display.sync operation timed out, skipping window detection."
                    )
                    return []
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.logger.debug(
                                f"Window geometry: ({geometry.x}, {geometry.y}) {geometry.width}x{geometry.height}"
                            )
                            self.logger.debug(
                                f"Translated coords: ({translated.x}, {translated.y})"
                            )

                            # For maximized windows or windows with negative coordinates,
                            # assume they start at (0, 0)
                            if (
                                geometry.width >= screen_width * 0.9
                                and geometry.height >= screen_height * 0.8
                            ):
                                # This looks like a maximized window
                                abs_x, abs_y = 0, 0
                                self.logger.debug(
                                    f"Window {title[:30]} appears maximized, using (0, 0)"
                                )
                            elif (
                                translated.x >= 0
                                and translated.y >= 0
                                and translated.y < screen_height
                            ):
                                ...
                                abs_x, abs_y = 0, 0
                                self.logger.debug(
                                    f"Window {title[:30]} has problematic coords, using (0, 0)"
                                )

                        except Exception as translate_error:
                            self.logger.debug(
                                f"Translation failed for {title[:30]}: {translate_error}, using (0, 0)"
                            )
                            abs_x, abs_y = 0, 0
                        ...
                        if geometry.width <= 0 or geometry.height <= 0:
                            self.logger.debug(
                                f"Window {title[:30]} has invalid dimensions: {geometry.width}x{geometry.height}"
                            )
                            continue

                    except Exception as geom_error:
                        self.logger.debug(
                            f"Could not get geometry for window {window}: {geom_error}"
                        )
                        continue
                    ...
                    self.logger.debug(
                        f"Found VS Code window: {title[:30]}... at ({abs_x},{abs_y}) size {geometry.width}x{geometry.height}"
                    )
    ...
    def _get_macos_windows(self) -> List[VSCodeWindow]:
        ...
        try:
            ...
            vscode_apps = [
                ...
                if any(
                    name in (app.bundleIdentifier() or "").lower()
    ...
    def _is_vscode_window(self, title: str) -> bool:
        ...
        # ENHANCED BROWSER EXCLUSION - Block common browser patterns
        browser_patterns = [
            "firefox",
            "chrome",
            "chromium",
        ...
        # Check for browser patterns first - immediate exclusion
        if any(pattern in title_lower for pattern in browser_patterns):
            self.logger.debug(f"❌ Excluding browser window: {title[:50]}")
            return False
        ...
        exclude_patterns = [
            ...
            "developer tools",
            "extension host",
            "terminal",  # System terminal windows
        ...
        result = has_vscode_indicator and not should_exclude
        if result:
            self.logger.debug(f"✅ Accepting VS Code window: {title[:50]}")
        else:
            self.logger.debug(f"❌ Rejecting window: {title[:50]}")
