...
class EnhancedButtonDetector:
    ...
    def _init_detection_modules(self):
        ...
        if HAS_EASYOCR and self.config.get("use_easyocr", True):
            try:
                # Configure PyTorch for CPU-only usage to suppress warnings
                import os

                os.environ["PYTORCH_DISABLE_GPU"] = "1"

                # Initialize EasyOCR with explicit CPU-only settings
                use_gpu = self.config.get("use_gpu", False)
                self.easyocr_reader = easyocr.Reader(
                    ["en"], gpu=use_gpu, download_enabled=False  # Prevent downloads
                )

                # Suppress DataLoader pin_memory warnings for CPU usage
                if not use_gpu:
                    import warnings
                ...
                self.logger.info(f"✅ EasyOCR initialized (GPU: {use_gpu})")
    ...
    def detect_continue_buttons(
        self, image: Image.Image, window_x: int = 0, window_y: int = 0
    ) -> List[EnhancedButtonLocation]:
        ...
        if image_hash in self.cache:
            ...
            return [
                self._adjust_button_coordinates(btn, window_x, window_y) for btn in cached_buttons
            ]
        ...
        if "easyocr" in self.available_methods:
            ...
            self.logger.debug(
                f"EasyOCR found {len(easyocr_buttons)} buttons in {detection_times['easyocr']:.2f}s"
            )
        ...
        if "enhanced_color" in self.available_methods:
            ...
            self.logger.debug(
                f"Enhanced color found {len(color_buttons)} buttons in {detection_times['enhanced_color']:.2f}s"
            )
        ...
        if "morphology" in self.available_methods:
            ...
            self.logger.debug(
                f"Morphology found {len(morph_buttons)} buttons in {detection_times['morphology']:.2f}s"
            )
        ...
        if "edge_detection" in self.available_methods:
            ...
            self.logger.debug(
                f"Edge detection found {len(edge_buttons)} buttons in {detection_times['edge_detection']:.2f}s"
            )
        ...
        if "yolo" in self.available_methods:
            ...
            self.logger.debug(
                f"YOLO found {len(yolo_buttons)} buttons in {detection_times['yolo']:.2f}s"
            )
        ...
        self.logger.info(
            f"🎯 Enhanced detection: {len(final_buttons)} buttons found in {total_time:.2f}s using {len(detection_times)} methods"
        )
            ...
            for color_name, color_info in color_ranges.items():
                ...
                for contour in contours:
                    ...
                    if 0.5 <= aspect_ratio <= 5.0:
                        ...
                        confidence = min(
                            0.9, 0.5 + color_info["confidence_boost"] + (area / 5000) * 0.2
                        )
    ...
    def _deduplicate_and_rank(
        self, buttons: List[EnhancedButtonLocation]
    ) -> List[EnhancedButtonLocation]:
    ...
    def _calculate_overlap(
        self, btn1: EnhancedButtonLocation, btn2: EnhancedButtonLocation
    ) -> float:
    ...
    def _cache_results(
        self, image_hash: str, buttons: List[EnhancedButtonLocation], window_x: int, window_y: int
    ):
    ...
    def _adjust_button_coordinates(
        self, button: EnhancedButtonLocation, window_x: int, window_y: int
    ) -> EnhancedButtonLocation:
