...
class ButtonFinder:
    ...
    def find_continue_buttons(
        ...
        try:
            ...
            if image.width <= 100 and image.height <= 100 and self._is_mock_image(image):
            ...
            if specific_buttons:
                ...
                if high_conf_specific:
                    self.logger.info(
                        f"🎯 Found {len(high_conf_specific)} high-confidence Continue buttons!"
                    )
            ...
            buttons = self._filter_non_continue_buttons(buttons, image.width, image.height)
                ...
                for config_name, config in ocr_configs:
                    try:
                        result = pytesseract.image_to_data(
                            proc_image, config=config, output_type=pytesseract.Output.DICT
                        )
                        ...
                        if word_count > 0:
                            self.logger.debug(
                                f"Preprocessing {proc_idx} + {config_name}: {word_count} words"
                            )

                            # Process the OCR results
                            found_buttons = self._process_ocr_results(
                                result, window_x, window_y, f"{config_name}_proc{proc_idx}"
                            )
                            buttons.extend(found_buttons)

                    except Exception as e:
                        self.logger.debug(
                            f"OCR error with {config_name} on preprocessing {proc_idx}: {e}"
                        )
                        continue
        except Exception as e:
    ...
    def _calculate_overlap(self, button1: ButtonLocation, button2: ButtonLocation) -> float:
        ...
        try:
            ...
            for y in range(0, height - 20, 10):  # Skip small increments
                for x in range(0, width - 50, 10):  # Skip small increments
                    ...
                    if (
                        abs(pixel[0] - target_blue[0]) < tolerance
                        and abs(pixel[1] - target_blue[1]) < tolerance
                        and abs(pixel[2] - target_blue[2]) < tolerance
                    ):
                        ...
                        if button_bounds:
                            ...
                            if (
                                40 <= bw <= 200 and 20 <= bh <= 60 and 1.5 <= bw / bh <= 6.0
                            ):  # Reasonable button proportions
                                ...
                                buttons.append(button)
                                self.logger.debug(
                                    f"Found blue rectangle at ({bx}, {by}) size {bw}x{bh}"
                                )
    ...
    def _trace_blue_rectangle(
        self, image: Image.Image, start_x: int, start_y: int, target_color: tuple, tolerance: int
    ) -> Optional[tuple]:
        ...
        try:
            ...
            # Expand right
            while right < width - 1 and self._color_matches(
                image.getpixel((right + 1, start_y)), target_color, tolerance
            ):
                right += 1

            # Expand left
            while left > 0 and self._color_matches(
                image.getpixel((left - 1, start_y)), target_color, tolerance
            ):
                left -= 1
            ...
            # Expand down
            while bottom < height - 1 and self._color_matches(
                image.getpixel((start_x, bottom + 1)), target_color, tolerance
            ):
                bottom += 1

            # Expand up
            while top > 0 and self._color_matches(
                image.getpixel((start_x, top - 1)), target_color, tolerance
            ):
                top -= 1
    ...
    def _find_blue_buttons(
        ...
        try:
            ...
            for contour in contours:
                ...
                if 50 < w < 150 and 20 < h < 50 and 1.5 < aspect_ratio < 5 and area > 800:
                    ...
                    if HAS_TESSERACT:
                        try:
                            ...
                            if text and any(
                                pattern in text for pattern in ["continue", "cont", "contin"]
                            ):
                                confidence = (
                                    0.9  # High confidence for blue button with Continue text
                                )
                                ...
                                buttons.append(button)
                                self.logger.debug(
                                    f"Found blue Continue button: '{text}' at ({x}, {y})"
                                )

                        except Exception as ocr_error:
                            self.logger.debug(
                                f"OCR failed for blue button at ({x}, {y}): {ocr_error}"
                            )
    ...
    def _process_ocr_results(
        self, result, window_x: int, window_y: int, method: str
    ) -> List[ButtonLocation]:
                ...
                if self._matches_continue_pattern(text):
                    ...
                    if confidence > 0.3:
                        ...
                        if 20 < w < 200 and 15 < h < 60:
                            ...
                            buttons.append(button)
                            self.logger.debug(
                                f"Found '{text}' button at ({x}, {y}) confidence: {confidence:.2f}"
                            )
    ...
    def _is_search_field_or_top_element(
        self, button: ButtonLocation, image_width: int, image_height: int
    ) -> bool:
        ...
        if button.y < top_area_height:
            self.logger.debug(
                f"Button at ({button.x}, {button.y}) is in top area "
        ...
        if aspect_ratio > 8:  # Very wide buttons are likely search fields
            self.logger.debug(
                f"Button at ({button.x}, {button.y}) has wide "
        ...
        if button.x < left_sidebar_width and button.y < image_height * 0.8:
            self.logger.debug(
                f"Button at ({button.x}, {button.y}) is in left " f"sidebar area, likely navigation"
            )
            return True
        ...
        if button.text:
            ...
            for term in search_terms:
                if term in text_lower:
                    self.logger.debug(
                        f"Button text '{button.text}' contains " f"search term '{term}'"
                    )
                    return True
    ...
    def _is_in_chat_panel_area(self, x: int, y: int, image_width: int, image_height: int) -> bool:
        ...
        if is_chat_area:
            ...
            self.logger.debug(
                f"Position ({x}, {y}) is NOT in chat panel area "
    ...
    def _detect_specific_continue_button(
        self, image: Image.Image, window_x: int, window_y: int
    ) -> List[ButtonLocation]:
        ...
        try:
            ...
            for contour in contours:
                ...
                if 50 < w < 100 and 20 < h < 45 and 1.5 < aspect_ratio < 4 and area > 800:
                    ...
                    if HAS_TESSERACT:
                        try:
                            ...
                            text = (
                                pytesseract.image_to_string(enhanced, config=config).strip().lower()
                            )

                            # Check for Continue text with some flexibility
                            if text and any(
                                pattern in text
                                for pattern in ["continue", "contin", "cont", "tinue"]
                            ):
                                ...
                                buttons.append(button)
                                self.logger.info(
                                    f"🎯 Found specific Continue button: '{text}' at ({x}, {y})"
                                )

                        except Exception as ocr_error:
                            self.logger.debug(
                                f"OCR failed for specific blue button at ({x}, {y}): {ocr_error}"
                            )
                        ...
                        if self._is_in_chat_panel_area(x, y, image.width, image.height):
                            ...
                            buttons.append(button)
                            self.logger.info(
                                f"🔵 Found blue Continue button candidate at ({x}, {y})"
                            )
    ...
    def _filter_non_continue_buttons(
        self, buttons: List[ButtonLocation], image_width: int = 0, image_height: int = 0
    ) -> List[ButtonLocation]:
        ...
        for button in buttons:
            ...
            if (
                image_width > 0
                and image_height > 0
                and self._is_search_field_or_top_element(button, image_width, image_height)
            ):
                self.logger.debug(
                    f"Excluding button at ({button.x}, "
            ...
            if image_width > 0 and image_height > 0:
                ...
                if not in_chat_area:
                    ...
                    if button.confidence < 0.7:
                        self.logger.debug(
                            f"Excluding low-confidence button "
            ...
            if not button.text:
                ...
                if (
                    image_width > 0
                    and image_height > 0
                    and not self._is_in_chat_panel_area(
                        button.x, button.y, image_width, image_height
                    )
                ):
                    self.logger.debug(
                        f"Excluding no-text button outside " f"chat area: ({button.x}, {button.y})"
                    )
                    continue
            ...
            for pattern in exclude_patterns:
                if re.search(pattern, text_lower, re.IGNORECASE):
                    self.logger.debug(
                        f"Excluding button '{button.text}' - " f"matches pattern '{pattern}'"
                    )
                    is_excluded = True
                    break
            ...
            if not is_excluded:
                ...
                if (
                    self._matches_continue_pattern(text_lower)
                    or not text_lower  # No text detected
                    or text_lower in ["", " ", "button", "btn"]
                ):
                    filtered.append(button)
                else:
                    self.logger.debug(
                        f"Excluding button '{button.text}' - " "doesn't match continue patterns"
                    )

        self.logger.debug(
            f"Filtered {len(buttons)} buttons down to " f"{len(filtered)} Continue candidates"
        )
        return filtered
